...
import { httpToolNames, httpTools, injectableHttpToolNames, injectHttpTools } from '@/lib/ai/tools/http-tools';
import { generateTitleFromUserMessage } from '../../actions';
import { realWorldToolNames, realWorldTools } from '@/lib/ai/tools/real-world-tools';
import { builtInToolNames, builtInTools } from '@/lib/ai/tools/built-in-tools';
import { registry } from '@/lib/ai/model-provider';
import { injectRagTools, RagTools } from '@/lib/ai/tools/rag-tools';
...
export async function POST(request: Request) {
  ...
  const { supportToolCalling = true } = model;
  ...
  const ragTools = isRetrieveKnowledge ? injectRagTools({ llm: customModel(model.apiIdentifier) }) : {};

  const allTools: typeof ALL_TOOLS = supportToolCalling ? [...ALL_TOOLS, ...(Object.keys(ragTools) as RagTools[])] : [];
